...
> 判断 eTLD+1 的方法依赖于公共后缀列表（Public Suffix List），参考[公共后缀列表](https://publicsuffix.org/list/)
>
...
注意：以上是**完整渲染管线**的流程。而[关键渲染路径(Critical Rendering Path)](https://developer.mozilla.org/en-US/docs/Web/Performance/Critical_rendering_path)特指从 HTML/CSS/JavaScript 到屏幕上首次渲染像素的关键阶段，仅包括：DOM 构建 → CSSOM 构建 → 渲染树构建 → 布局 → 绘制。合成、光栅化等 GPU 阶段通常不纳入关键渲染路径，因为它们不阻塞首次渲染。
...
- **脚本处理规则**：
  - 遇到`<script>`时暂停 DOM 构建并执行脚本，如果脚本需要下载，则需要等待脚本下载并执行完成。（除非标记`async`/`defer`）
    > async/defer 的区别，参考： [脚本：async，defer](https://zh.javascript.info/script-async-defer#zong-jie)
...
- **预解析扫描器（Preload Scanner）**
  ...
  - 主解析器在解析 HTML 文档时，会启动一个轻量级的扫描器（独立线程）提前发现并下载 CSS/JS/图片资源
    > 关于预解析扫描器是否是一个独立的线程，存在争议。但可以肯定的是，预解析扫描器与主解析器之间是并行的。参考[HTML 标准](https://html.whatwg.com.cn/multipage/parsing.html#active-speculative-html-parser)。
...
| 相对单位 | 转换依据                                                                                  | 示例                           |
| -------- | ----------------------------------------------------------------------------------------- | ------------------------------ |
| em       | - 用于普通属性时，相对于本元素的字体大小 <br> - 用于 `font-size` 时相对于父元素的字体大小 |                                |
...
**注意**： 伪元素的样式在样式计算阶段确定，但对应的渲染节点在后续渲染树构建时才生成。
